...
<body class="bg-white">
    ...
    <div class="max-w-7xl mx-auto my-8 rounded-2xl overflow-hidden shadow-lg flex flex-col gap-2 bg-white">
        ...
        <div class="py-16 px-8" style="background-color: #90B7BF;">
            <div class="max-w-7xl mx-auto">
                <h2 class="text-center text-xl font-bold text-[#122B1D] mb-12 tracking-widest">EVENTS</h2>

                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @forelse($events as $event)
                        <a href="#" class="block bg-white rounded-lg overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300 ease-in-out">
                            <div class="h-56 bg-cover bg-center"
                                style="background-image: url('{{ $event->poster_link }}');">
                            </div>

                            <div class="p-6 bg-white">
                                <h3 class="text-xl font-bold text-gray-900 mb-2 group-hover:text-indigo-600 transition-colors duration-300">{{ $event->title }}</h3>
                                ...
                                <div class="flex items-center text-gray-600 text-sm mt-4">
                                    ...
                                    <span class="font-medium">{{ $startTime->isoFormat('D MMM YYYY') }}
                                        @if ($endTime && $startTime->diffInDays($endTime) > 0)
                    ...
                    @empty
                        <div class="col-span-full text-center py-16">
                            ...
                            <p class="mt-1 text-sm text-gray-500">Saat ini belum ada event yang terjadwal. Silakan cek kembali nanti.</p>
                        </div>
                    @endforelse
                </div>
            </div>
