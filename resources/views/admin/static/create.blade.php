...
<div class="p-6 min-h-screen bg-gray-50 flex items-center justify-center">
  ...
  <div class="w-full max-w-2xl bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
    <h1 class="text-2xl font-semibold text-gray-800 mb-6">Tambah Embed Instagram</h1>
    ...
    {{-- Form --}}
    <form action="{{ route('admin.statics.store') }}" method="POST" class="space-y-5">
      @csrf
      ...
      <div>
        ...
        <textarea name="embed_code" id="embed_code" rows="7"
          placeholder="Tempelkan kode embed di sini..."
          class="w-full rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-400 focus:outline-none px-4 py-2 text-sm text-gray-800">{{ old('embed_code') }}</textarea>
      </div>
